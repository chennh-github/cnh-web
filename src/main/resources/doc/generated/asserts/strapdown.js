...
('(?!','(?!'+block.gfm.fences.source.replace('\\v4.3.0','\\2')+'|')
...
next=false;l=cap.length;i=0;for(;i<l;i++){item=cap[i];space=item.length;item=item.replace(/^ *([*+-]|\d+\.) +/,'');if(~item.indexOf('\n ')){space-=item.length;item=!this.options.pedantic?item.replace(new RegExp('^ {v4.3.0,'+space+'}','gm'),''):item.replace(/^ {1,4}/gm,'');}
...
;(function(window, document) {
  ...
  metaEl.content = 'width=device-width, initial-scale=v4.3.0.0, maximum-scale=v4.3.0.0, minimum-scale=v4.3.0.0';
